...
def mpc_control(state, obstacle_positions, dt, left_turn=False):
    ...
    for steer in steer_candidates:
        ...
        for _ in range(horizon):
            ...
            yaw_trailer += target_speed / L_t * np.sin(pyaw - yaw_trailer) * dt
            py_trailer = py - L_t * np.sin(yaw_trailer)

            lane_error = target_lane - py
            yaw_ref = np.clip(0.6 * lane_error, -0.5 if left_turn else -0.3,
                              0.5 if left_turn else 0.3)
...
def parking_control(state):
    ...
    if not depth_reached:
        ...
        yaw_err = np.arctan2(np.sin(final_yaw - yaw), np.cos(final_yaw - yaw))
...
# PARKING LOCK
# ==========================================================
def apply_parking_lock(car_id):
    p.changeDynamics(car_id, -1, linearDamping=1.0, angularDamping=1.0)
    p.resetBaseVelocity(car_id, [0, 0, 0], [0, 0, 0])
    for j in range(p.getNumJoints(car_id)):
        p.changeDynamics(car_id, j, lateralFriction=5.0, rollingFriction=5.0, spinningFriction=5.0)
        p.setJointMotorControl2(car_id, j, p.VELOCITY_CONTROL, targetVelocity=0.0, force=300.0)
...
def main():
    ...
    park_counter = 0
    timeout_counter = 0
    PARK_CONFIRM_STEPS = 6
    TIMEOUT_STEPS = float(0.5 / env.dt)  # approx 15 seconds
    ...
    try:
        for step in range(3000):
            ...
            # -------------------------------
            # PARKING TERMINATION (GRADE TWO)
            # -------------------------------
            if mode == "PARKING":
                x, y, yaw = obs
                fully_inside = x > PARKING_X + PARKING_LENGTH / 2 - 0.5
                if fully_inside:
                    park_counter += 1
                    timeout_counter += 1
                else:
                ...
                aligned = abs(y - 6.0) < 0.25 and abs(yaw) < 0.06
                if aligned and park_counter >= PARK_CONFIRM_STEPS:
                    print("GRADE TWO PARKING")
                    mode = "PARKED"
                    env.parked = True
                    apply_parking_lock(car_id)
                elif timeout_counter >= TIMEOUT_STEPS:
                    print("GRADE TWO PARKING (timeout)")
                    mode = "PARKED"
                    env.parked = True
                    apply_parking_lock(car_id)
            ...
            p.resetDebugVisualizerCamera(cameraDistance=8.0, cameraYaw=270,
                                         cameraPitch=-20, cameraTargetPosition=car_pos)
